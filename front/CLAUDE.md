...
- **Default language on load** (if bilingual): Arabic
...
- **Animation**: Basic (CSS/Transition) — elegant and premium, not flashy; nothing advanced
  installed yet, don't add a library without asking (see Constraints)
...
أضف أنيميشن بناءً على اختيار المشروع في **Project Overview** بس — متفترضش إنه مطلوب دايمًا.

- **None**: متضيفش أي أنيميشن غير الأساسيات المتاحة جاهزة (زي `<Transition>` لو موجود بالفعل في الكود).
- **Basic**: استخدم Tailwind transitions و`<Transition>` / `<TransitionGroup>` وCSS keyframes بس.
- **Advanced**: لو المشروع محدد Advanced أو المستخدم طلب صراحة، استخدم AOS/GSAP/@vueuse/motion (لو مثبتة في `package.json` بالفعل أو المستخدم وافق على تثبيتها).

لو الاختيار مش محدد في Project Overview، اسأل المستخدم الأول قبل ما تضيف أي مكتبة أو أنيميشن متقدم.
...
اتبع الاختيار المحدد في **Project Overview** (Language + Default language on load) — متفترضش عربي دايمًا. لو مش محدد، اسأل المستخدم قبل ما تبدأ.

- **Arabic-only**: `dir="rtl"` و`lang="ar"` ثابتين على `<html>`. مفيش toggle للغة.
- **English-only**: `dir="ltr"` و`lang="en"` ثابتين على `<html>`. مفيش toggle للغة، ومفيش داعي لـ RTL classes.
- **Arabic + English**: ابدأ بالـ **Default language** المحدد في Project Overview (عربي أو إنجليزي) — خزّن اللغة الحالية في `localStorage` + Pinia store، وبدّل `dir`/`lang` على `<html>` ديناميكيًا لما المستخدم يغيّر اللغة.
- Tailwind RTL: استخدم `rtl:` modifier أو logical properties (`ms-`, `me-`, `ps-`, `pe-`, `start`, `end`) في أي مشروع فيه RTL (Arabic-only أو bilingual).
- Translations: استخدم `vue-i18n` أو static object حسب حجم المشروع (للمشاريع bilingual بس).
